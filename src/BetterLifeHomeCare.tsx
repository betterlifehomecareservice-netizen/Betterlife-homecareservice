...
function Reveal({ children, delay = 0, className = "" }: { children: React.ReactNode; delay?: number; className?: string }) {
  ...
  return (
    <div ref={ref} className={className} style={{ opacity: inView ? 1 : 0, transform: inView ? "translateY(0)" : "translateY(24px)", transition: `all 700ms cubic-bezier(0.22,1,0.36,1) ${delay}s` }}>
      {children}
    </div>
  );
}
...
const PRODUCTS = [
  { id: 1, nameBn: "হাসপাতাল বেড (স্ট্যান্ডার্ড)", nameEn: "Hospital Bed (Standard)", priceBn: "৳২২,০০০", priceEn: "৳22,000", emoji: "🛏️", bg: "#f0fdf4", color: "#10b981",
    descBn: "Manual adjustable, sturdy frame, mattress included", descEn: "Manual adjustable, sturdy frame, mattress" },
  { id: 2, nameBn: "নেবুলাইজার মেশিন", nameEn: "Nebulizer Machine", priceBn: "৳৩,৫০০", priceEn: "৳3,500", emoji: "🌬️", bg: "#ecfdf5", color: "#059669",
    descBn: "Compressor nebulizer, quiet operation, mask set included", descEn: "Compressor type, quiet, complete mask set" },
  { id: 3, nameBn: "অক্সিজেন সিলিন্ডার", nameEn: "Oxygen Cylinder", priceBn: "৳৮,০০০", priceEn: "৳8,000", emoji: "🔵", bg: "#f5f3ff", color: "#8b5cf6",
    descBn: "Medical grade, regulator & mask included", descEn: "Medical grade, regulator & mask included" },
  { id: 4, nameBn: "BP মনিটর (ডিজিটাল)", nameEn: "BP Monitor (Digital)", priceBn: "৳২,৮০০", priceEn: "৳2,800", emoji: "❤️", bg: "#fdf2f8", color: "#ec4899",
    descBn: "Upper arm auto BP & pulse monitor, 90 reading memory", descEn: "Upper arm auto BP & pulse, 90 reading memory" },
  { id: 5, nameBn: "হুইলচেয়ার (ফোল্ডেবল)", nameEn: "Wheelchair (Foldable)", priceBn: "৳১২,০০০", priceEn: "৳12,000", emoji: "♿", bg: "#fef9c3", color: "#ca8a04",
    descBn: "Foldable lightweight aluminum, footrest & armrest included", descEn: "Foldable aluminum, footrest & armrest" },
]
const REVIEWS = [
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
            {filteredProducts.map((p, i) => (
              <Reveal key={p.id} delay={i * 0.04}>
                <div className="card-hover rounded-2xl border border-slate-200 bg-white p-4 shadow-sm flex flex-col h-full">
                  {/* Product visual */}
                  <div className="h-24 rounded-xl flex items-center justify-center mb-3 text-4xl" style={{ background: p.bg }}>
                    {p.emoji}
                  </div>
                  <h3 className="font-bold text-sm text-slate-900 mb-1 leading-tight">{isBn ? p.nameBn : p.nameEn}</h3>
                  <p className="text-[11px] text-slate-400 leading-relaxed flex-1 mb-3">{isBn ? p.descBn : p.descEn}</p>
                  <div className="font-extrabold text-base mb-3" style={{ color: p.color }}>{isBn ? p.priceBn : p.priceEn}</div>
                  <button onClick={() => setOrderModal({ open: true, product: { name: isBn ? p.nameBn : p.nameEn, price: isBn ? p.priceBn : p.priceEn } })}
                    ...
                    <ShoppingCart className="h-3.5 w-3.5" />{isBn ? "অর্ডার করুন" : "Order Now"}
                  </button>
                </div>
              </Reveal>
            ))}
